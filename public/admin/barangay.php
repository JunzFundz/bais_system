...
<div class="flex flex-col md:flex-row md:items-end gap-4 gap-x-4 gap-y-4 justify-between">
    <div class="">
        ...
        <p class="text-slate-500 font-poppins">All barangay in Bais City</p>
    </div>
</div>
<br>
<!-- Stats Cards Grid -->
        ...
        foreach ($allbrgy as $row):
            ...
            <div class="w-full max-w-sm px-4 py-3 bg-white rounded-md shadow-md dark:bg-gray-800">
                <div class="flex items-center justify-between">
                    <span class="px-3 py-1 text-xs text-green-800 uppercase bg-green-200 rounded-full dark:bg-green-300 dark:text-green-900">active</span>
                </div>

                <div>
                    <h1 class="mt-2 text-lg font-semibold text-gray-800 dark:text-white"><?= htmlspecialchars($_SESSION['BARANGAY']) ?></h1>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">Total registered: <?= htmlspecialchars($row['total_persons']) ?></p>
                </div>

                <div>
                    <div class="flex items-center mt-2 text-gray-700 dark:text-gray-200">
                        <a href="officials?barangay=<?= htmlspecialchars($row['BRGY_ID']) ?>" class="mx-2 text-blue-600 cursor-pointer dark:text-blue-400 hover:underline" tabindex="0" role="link">View in tab</a>
                    </div>
